...
public class Logic {
	...
	int[] IRvalues = new int[48];
	
	int[] lineposition = new int[2];
	float szaberror = 0;
	int servo_speed_param_selector = 0;
	//int[] servo_speed_params = new int[18];
	int[] servo_speed_params = {5, 0, 0, 100,    15, 0, 0, 100,  6,		//slow
								3, 0, 0, 100,    10, 0, 40, 100,  6};  	//faster
	int servo_speed_param_slow = 20;
	int servo_speed_param_faster = 30;
	
	
	volatile int drivervoltage =0;
	volatile int motorvoltage =0;
	...
	Leg_movement_editor move_editor = null;
	...
	public Logic(MISE_protocol m){
		misep = m;
		startRunningThread();
		move_editor = new Leg_movement_editor(misep);
		//startMatlabThread();
	}
	void connect(){
		misep.connect();
	...
	void Send_Stop(){
		...
		System.out.println("TODO SEND STOP SIGNAL not implemented yet");
	}
	
	void Send_servo_position(int channel, int position, int delay){
		byte[] data={(byte) channel, (byte) position, (byte) delay};
		misep.send_msg( 54, data,  3);
	}
	
	void Send_supply_ONOFF(int value){
		byte[] data={(byte) value};
		misep.send_msg( 52, data,  1);
	}
	...
	void data_process() {
		...
		case 3: {
			...
			supply_servo_state = misep.incoming_message.removeFirst();
			
			break;
		}
		case 24: {
			Leg_currents[0] = misep.incoming_message.removeFirst();
			Leg_currents[0] += 256 * (misep.incoming_message.removeFirst());
			Leg_currents[0] += 256 * 256 * (misep.incoming_message.removeFirst());
			Leg_currents[0] += 256 * 256 * 256 * (misep.incoming_message.removeFirst());

			Leg_currents[1] = misep.incoming_message.removeFirst();
			Leg_currents[1] += 255 * (misep.incoming_message.removeFirst());
			Leg_currents[1] += 256 * 256 * (misep.incoming_message.removeFirst());
			Leg_currents[1] += 256 * 256 * 256 * (misep.incoming_message.removeFirst());

			Leg_currents[2] = misep.incoming_message.removeFirst();
			Leg_currents[2] += 256 * (misep.incoming_message.removeFirst());
			Leg_currents[2] += 256 * 256 * (misep.incoming_message.removeFirst());
			Leg_currents[2] += 256 * 256 * 256 * (misep.incoming_message.removeFirst());

			Leg_currents[3] = misep.incoming_message.removeFirst();
			Leg_currents[3] += 256 * (misep.incoming_message.removeFirst());
			Leg_currents[3] += 256 * 256 * (misep.incoming_message.removeFirst());
			Leg_currents[3] += 256 * 256 * 256 * (misep.incoming_message.removeFirst());

			Leg_currents[4] = misep.incoming_message.removeFirst();
			Leg_currents[4] += 256 * (misep.incoming_message.removeFirst());
			Leg_currents[4] += 256 * 256 * (misep.incoming_message.removeFirst());
			Leg_currents[4] += 256 * 256 * 256 * (misep.incoming_message.removeFirst());
			
			Leg_currents[5] = misep.incoming_message.removeFirst();
			Leg_currents[5] += 256 * (misep.incoming_message.removeFirst());
			Leg_currents[5] += 256 * 256 * (misep.incoming_message.removeFirst());
			Leg_currents[5] += 256 * 256 * 256 * (misep.incoming_message.removeFirst());
			
			for(int i=0; i<6;i++){
				Leg_color[i] = Leg_currents[i];
			}
		...
		case 20: {
			...
			/*
			increment += 256 * 256 * 256 * 256 * (misep.incoming_message.removeFirst());
			increment += 256 * 256 * 256 * 256 * 256 * (misep.incoming_message.removeFirst());
			increment += 256 * 256 * 256 * 256 * 256 * 256 * (misep.incoming_message.removeFirst());
			increment += 256 * 256 * 256 * 256 * 256 * 256 * 256 * (misep.incoming_message.removeFirst());
			*/
			break;
		}
		case 21: {
			/*speed[0] = misep.incoming_message.removeFirst();
			speed[0] += 256 * (misep.incoming_message.removeFirst());
			speed[0] += 256 * 256 * (misep.incoming_message.removeFirst());
			speed[0] += 256 * 256 * 256 * (misep.incoming_message.removeFirst());

			speed[1] = misep.incoming_message.removeFirst();
			speed[1] += 255 * (misep.incoming_message.removeFirst());
			speed[1] += 256 * 256 * (misep.incoming_message.removeFirst());
			speed[1] += 256 * 256 * 256 * (misep.incoming_message.removeFirst());
			*/
			 byte[] bytes = {0,0,0,0};
			 int temp=0;
			 temp=misep.incoming_message.removeFirst();
...
//				stringlock.unlock();
		...
		case 22: {
			battery_voltage = misep.incoming_message.removeFirst();
			battery_voltage += 256 * (misep.incoming_message.removeFirst());
			battery_voltage += 256 * 256 * (misep.incoming_message.removeFirst());
			battery_voltage += 256 * 256 * 256 * (misep.incoming_message.removeFirst());
