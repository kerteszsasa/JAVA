...
public class MISE_protocol {
	...
	//Misep special chars
	boolean newcapvalue =false; //TODO remove this
	...
	// szonda msg varibles 
	int szonda_msg_LENGTH;
	LinkedList<Byte> szonda_incoming_message_bytes = new LinkedList<Byte>();
	
	boolean process_szonda_message(){
		//System.out.println("feldolgoz");
				serialport.lock.lock();
			/*	if(serialport.incoming_data_fifo.isEmpty()){
					serialport.lock.unlock();
				...
				if(serialport.incoming_data_fifo.size() < 2){
					serialport.lock.unlock();
					return false;
				}
			...
			/* msg_TO = serialport.incoming_data_fifo.remove();
				...
				/* int datasize = (int)msg_LENGTH1& 0xFF ;
				 ...
				 serialport.lock.unlock();
				 //wait for get all data + CRC
				 while(available_data <  szonda_msg_LENGTH+1 ){	// data + 1 byte CRC
				 ...
				 szonda_incoming_message_bytes.clear();
				 
				 byte data;
				 byte crc= (byte)szonda_msg_LENGTH;
				 //TODO shiftelni !!!
				 for(int i=0;i< ( szonda_msg_LENGTH) ;i++){
					 data = serialport.incoming_data_fifo.remove();
					 crc+=data;
					 
					 //TODO 
					 //TODO  itt kivonok az eredménybõl 50-et, hogy meg tudjam jeleníteni, ez csalás!!!!
					 data-=50;
					 
					 szonda_incoming_message_bytes.add(data);
				 }
				 ...
				 ////////////**/
				/* int touch = (int)   szonda_incoming_message_bytes.removeFirst()          & 0xFF ;
				 
				 if(touch > 136) System.out.print(    touch     );
				 System.out.print("               ");
				 for (int i=0; i<touch-125;i++ )System.out.print("0");
				 System.out.println("");
